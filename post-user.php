<?php
  ...
  if (!isLoggedIn() || isAdmin()) {
    // Only regular users can post here (admins use post-admin.php)
    if (!isLoggedIn()) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($phone !== '' && !preg_match('/^[\d\s\-\+\(\)]+$/', $phone)) {
      ...
      try {
        ...
        // User posts default to pending moderation and inactive state until approved
        $stmt = $pdo->prepare('INSERT INTO annonces (titre, description, date_publication, etat, moderation_status, image_path, id_utilisateur, prix, phone, ville, categorie) VALUES (?, ?, NOW(), "inactive", "pending", ?, ?, ?, ?, ?, ?)');
        $stmt->execute([$titre, $description, $imagePath, currentUserId(), $prix, $phone !== '' ? $phone : null, $ville !== '' ? $ville : null, $categorie !== '' ? $categorie : null]);
        simulateAfterInsertAnnonce($pdo, currentUserId());
  ...
  <section class="container post-grid">
    ...
    <div class="post-card">
      <form id="post-form" class="simple-form" method="POST" enctype="multipart/form-data">
        ...
        <div class="two">
          <input class="input" id="price" name="price" type="number" step="0.01" min="0" placeholder="Price (optional)" />
        ...
        <div class="actions">
          ...
          <a class="btn ghost" href="user-consult-user.php">Cancel</a>
        </div>
      </form>
    </div>
