<?php
  ...
  $cards = [];
  try {
    // Admin can see all ads (approved, pending, rejected) for moderation purposes
    $stmt = $pdo->query("SELECT a.id, a.titre, a.description, a.date_publication, a.etat, a.image_path, a.moderation_status, a.prix, a.ville, a.categorie, u.nom AS auteur
                          FROM annonces a JOIN utilisateurs u ON a.id_utilisateur = u.id
                          ORDER BY a.date_publication DESC LIMIT 12");
    $cards = $stmt->fetchAll();
  } catch (Throwable $e) {
    $cards = [];
  }
...
<head>
  ...
  <title>QuickAnnonce - Admin Dashboard</title>
...
<body class="grid">
  ...
  <header class="header">
    <div class="container header-inner">
      ...
      <!-- NAVIGATION -->
      <nav class="nav">
        <a class="active" href="index-admin.php">Home</a>
    ...
    <div class="container banner-inner">
      <div>
        ...
        <div style="margin-top:12px; display:flex; gap:10px;">
          <a class="btn" href="user-consult-admin.php">Browse Listings</a>
          <a class="btn secondary" href="post-admin.php">Post Your Ad</a>
        </div>
  ...
  <main class="container">
    <div class="section-title">
      ...
      <a class="btn ghost" href="user-consult-admin.php">See all</a>
    </div>
            ...
            <div class="card-body">
              ...
              <div class="card-meta">
                <?php if (!empty($c['prix']) && $c['prix'] > 0): ?>
                  ...
                  <span>•</span>
                <?php endif; ?>
                By <?php echo h($c['auteur']); ?> • <?php echo (int)max(0, (new DateTime())->diff(new DateTime($c['date_publication']))->days); ?> days ago • 
                <span style="padding: 2px 6px; border-radius: 3px; font-size: 0.75rem; background: <?php echo $c['moderation_status']==='approved'?'#d1fae5':($c['moderation_status']==='pending'?'#fef3c7':'#fee2e2'); ?>; color: <?php echo $c['moderation_status']==='approved'?'#065f46':($c['moderation_status']==='pending'?'#92400e':'#991b1b'); ?>;">
                  <?php echo ucfirst($c['moderation_status'] ?? 'pending'); ?>
                </span>
              ...
              <div class="card-actions">
                ...
                <a class="btn" href="user-consult-admin.php">More</a>
              </div>
            </div>
          </article>
        <?php endforeach; ?>
